...
try {
    ...
    Write-Host "    [OK] Config loaded from: $ConfigPath" -ForegroundColor Green
} catch {
    Write-Host "    [X] ERROR: Failed to load config: $_" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $lockFile) {
    Write-Host "    [WARN] Lock file exists: $lockFile" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($processAlive) {
            Write-Host "    [X] Process is still running - lock is ACTIVE" -ForegroundColor Red
            $checksFailed += "Lock file is active (process running)"
        } else {
            Write-Host "    [WARN] Process is dead - checking if lock is stale..." -ForegroundColor Yellow
            
            $lockTimeoutHours = $config.collision_control.lock_timeout_hours
            if ($lockAgeHours -gt $lockTimeoutHours) {
                Write-Host "    [OK] Lock is STALE (>$lockTimeoutHours hours, process dead)" -ForegroundColor Green
        ...
        Write-Host "    [X] ERROR reading lock file: $_" -ForegroundColor Red
...
foreach ($taskName in $config.scheduled_tasks) {
    try {
        ...
        Write-Host "    [WARN] Could not check task: $taskName" -ForegroundColor Yellow
    }
}

if ($tasksRunning.Count -gt 0) {
    Write-Host "    [X] Running tasks detected: $($tasksRunning -join ', ')" -ForegroundColor Red
    $checksFailed += "Scheduled tasks are running"
} else {
    Write-Host "    [OK] No scheduled tasks running" -ForegroundColor Green
    $checksPassed++
}
...
if ($arcProcesses) {
    Write-Host "    [WARN] ArcGIS-related processes detected:" -ForegroundColor Yellow
    ...
    if ($gpWorkers) {
        Write-Host "    [X] Active geoprocessing workers detected - BLOCKING" -ForegroundColor Red
        $checksFailed += "Geoprocessing workers are active"
    } else {
        Write-Host "    [OK] ArcGIS Pro is open but no active geoprocessing" -ForegroundColor Green
        $checksPassed++
    }
} else {
    Write-Host "    [OK] No ArcGIS processes running" -ForegroundColor Green
    $checksPassed++
}
...
try {
    ...
    Write-Host "    [OK] Geodatabase is accessible (not locked)" -ForegroundColor Green
    $checksPassed++
} catch {
    Write-Host "    [X] Geodatabase appears to be locked: $_" -ForegroundColor Red
...
if (-not (Test-Path $BackfillFile)) {
    Write-Host "    [X] Backfill file not found: $BackfillFile" -ForegroundColor Red
    $checksFailed += "Backfill file not found"
} else {
    $fileSize = (Get-Item $BackfillFile).Length / 1MB
    Write-Host "    [OK] File exists: $BackfillFile" -ForegroundColor Green
    Write-Host "    File size: $([int]$fileSize) MB" -ForegroundColor Cyan
    
    # Check file size is reasonable
    if ($fileSize -lt 10) {
        Write-Host "    [WARN] WARNING: File size seems small for backfill data" -ForegroundColor Yellow
    }
    ...
    if (-not $SkipSheetCheck) {
        ...
        try {
            ...
            if ($sheetNames -contains $requiredSheet) {
                Write-Host "    [OK] Required sheet '$requiredSheet' found" -ForegroundColor Green
                $checksPassed++
            } else {
                Write-Host "    [X] Required sheet '$requiredSheet' NOT found" -ForegroundColor Red
            ...
            Write-Host "    [WARN] WARNING: Could not validate sheet name: $_" -ForegroundColor Yellow
            Write-Host "    Assuming sheet name is correct..." -ForegroundColor Yellow
            $checksPassed++
        }
    } else {
        Write-Host "    [WARN] Sheet name check skipped" -ForegroundColor Yellow
        $checksPassed++
    }
}
...
if ($freeSpaceGB -lt 5) {
    Write-Host "    [X] Low disk space (< 5 GB)" -ForegroundColor Red
...
if ($checksFailed.Count -gt 0) {
    Write-Host ""
    Write-Host "[X] FAILED CHECKS:" -ForegroundColor Red
    ...
    Write-Host ""
    Write-Host "[OK] ALL CHECKS PASSED - Ready for backfill publish" -ForegroundColor Green
    Write-Host ("=" * 70) -ForegroundColor Cyan
    exit 0
}
